...
func _apply_box_settings() -> void:
	var dialog_text_panel: NinePatchRect = %DialogTextPanel
	...
	if box_color_use_global:
		dialog_text_panel.self_modulate.a = 255
	else:
		dialog_text_panel.self_modulate.a = 255
...
func _apply_name_label_settings() -> void:
	...
	if ResourceLoader.exists(name_label_box_panel):
		name_label_panel.add_theme_stylebox_override(&'panel', load(name_label_box_panel) as StyleBoxTexture)
	else:
		name_label_panel.add_theme_stylebox_override(&'panel', load(this_folder.path_join("vn_textbox_name_label_panel.tres")) as StyleBoxTexture)

	if name_label_box_use_global_color:
		name_label_panel.self_modulate.a = 255 
	else:
		name_label_panel.self_modulate.a = 255
	var dialog_text_panel: NinePatchRect = %DialogTextPanel
